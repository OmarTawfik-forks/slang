...
pub fn setup() -> Dependencies {
    let binding_graph = super::init_bindings::run(super::init_bindings::setup());
    let files = super::parser::run(super::parser::setup());

    Dependencies { binding_graph, files }
}
...
pub fn run(dependencies: Dependencies) -> BindingGraph {
    ...
    } in &files
    {
        ...
        definition_count += binding_graph.all_definitions().count();
    }
