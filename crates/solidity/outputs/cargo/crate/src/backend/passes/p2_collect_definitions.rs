...
impl Visitor for Pass {
    ...
    fn enter_path_import(&mut self, node: &input_ir::PathImport) -> bool {
        ...
        if let Some(alias) = &node.alias {
            let definition = Definition::new_import(node.id(), &alias.identifier, imported_file_id);
            self.insert_definition_in_current_scope(definition);
    ...
    fn enter_named_import(&mut self, node: &input_ir::NamedImport) -> bool {
        ...
        let definition =
            Definition::new_import(node.id(), &node.alias.identifier, imported_file_id);
        self.insert_definition_in_current_scope(definition);
    ...
    fn enter_modifier_definition(&mut self, node: &input_ir::ModifierDefinition) -> bool {
        let definition = Definition::new_modifier(node.id(), &node.name);
        self.insert_definition_in_current_scope(definition);
