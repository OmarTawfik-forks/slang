use crate::wasm_crate::utils::{define_rc_wrapper, define_wrapper, FromFFI, IntoFFI};
...
mod ffi {
    pub use crate::wasm_crate::bindgen::exports::nomic_foundation::slang::bindings::{
        ...
        Definition, DefinitionBorrow, Guest, GuestBindingGraph, GuestBuiltInLocation,
...
define_rc_wrapper! { BindingGraph {
    fn definition_at(&self, cursor: ffi::Cursor) -> Option<ffi::Definition> {
        self._borrow_ffi()
            .definition_at(&cursor._from_ffi())
            .map(rust::Definition::from)
            .map(IntoFFI::_into_ffi)
    }

    fn reference_at(&self,  cursor: ffi::Cursor) -> Option<ffi::Reference> {
        self._borrow_ffi()
            .reference_at(&cursor._from_ffi())
