...
define_rc_wrapper! { NonterminalNode {
    fn new(kind: ffi::NonterminalKind, children: Vec<ffi::Edge>) -> NonterminalNodeWrapper {
        let children = children.into_iter().map(FromFFI::_from_ffi).collect();

        NonterminalNodeWrapper(rust::NonterminalNode::new(kind._from_ffi(), children))
    }
...
define_rc_wrapper! { TerminalNode {
    fn new(kind: ffi::TerminalKind, text: String) -> TerminalNodeWrapper {
        TerminalNodeWrapper(rust::TerminalNode::new(kind._from_ffi(), text))
    }
