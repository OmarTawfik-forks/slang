...
pub fn generate_syntax_nodes_mod(
    ...
    let templates_root_dir =
        CargoWorkspace::locate_source_crate("codegen_syntax").join("src/nodes");

    let mut codegen = Codegen::read_write(&templates_root_dir)?;

    let template_path = templates_root_dir.join("kinds/template.tera");
