...
pub fn generate_supported_versions_page(manifest: &Manifest) -> NavigationEntry {
    let versions = &manifest.versions;
    let version_breaks = manifest.collect_version_breaks();
    ...
    page.write_newline();
    page.write_text(&format!("This specification compiles information from {all_count} publicly released versions of {language}. Among which, {breaks_count} versions had syntax-related changes:", all_count = versions.len(), language = manifest.title, breaks_count = version_breaks.len()));
...
pub fn generate_grammar_dir(
    ...
    for version in manifest.collect_version_breaks() {
        pages.push(NavigationEntry::Page {
            title: format!("v{version}"),
            path: format!("v{version}"),
            contents: generate_grammar_page(manifest, snippets, repo_root, &version),
        });
    }
